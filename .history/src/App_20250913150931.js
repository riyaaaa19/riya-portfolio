...
import TimelineCard from "./components/TimelineCard"; // adjust path if in /components
    ...
    <div>
      ...
      {activePage === "certificates" && (
        <section>
          ...
          {certificates.map((c, i) => (
            <TimelineCard key={i} title={c.title} icon="📜" />
          ))}
        </section>
      )}
      ...
      {activePage === "awards" && (
        <section>
          <h2>Awards</h2>
          {awards.map((a, i) => (
            <TimelineCard key={i} title={a.title} icon="🏆" />
          ))}
        </section>
      )}
